...
% read in obs data
 obsdata=textread('obs_outputs_0');
...
% specify assumed observation errors here
 for ii=1:n, yobs(ii).Sigy=diag((0.004^2)*ones(length(yobs(ii).y),1)); end
% scale obs x
...
% compute on simulations
 ...
 if(doPlot)
   ...
   figure(1); print -dpng fdPc; close;
 end
...
% plot sims and data
 if (doPlot)
   ...
   figure(2); print -dpng fdDat; close;
 end
